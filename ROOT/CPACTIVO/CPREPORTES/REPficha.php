...
$usuario = $_SESSION["codigo"];
   ...
   // encabezados
	...
	$pdf->SetAligns(array('C', 'C', 'C', 'C', 'C', 'C'));  // AQUÍ LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;$pdf->SetFont('Arial','B',6);  // AQUI LE ASIGNO EL TIPO DE LETRA Y TAMAÑO
	$pdf->SetFillColor(236, 240, 244);
   $pdf->SetDrawColor(145, 145, 145);
   $pdf->SetTextColor(74, 74, 74); // gris
	...
	////////////////////////////////////// CUERPO ///////////////////////////////////////////
	$pdf->SetWidths(array(7, 90, 30, 30, 20, 30));  // AQUÍ LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
	$pdf->SetAligns(array('C', 'L', 'C', 'C', 'C', 'C'));  // AQUÍ LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;$ClsFal = new ClsFalla();
	$result = $ClsFal->get_falla('',$codigo);$i=1;
	if(is_array($result)){
		foreach($result as $row){
			//nombre
			$nombre = utf8_decode($row["act_nombre"]);
