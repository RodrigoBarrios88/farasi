...
$id = $_SESSION["codigo"];
	...
	$pdf->MultiCell(0, 5, 'Reporte de Fallas', 0 , 'L' , 0);
   ...
   $pdf->SetTextColor(0);  // AQUI LE DOY COLOR AL TEXTO
	...
	////////////////////////////////////// CUERPO ///////////////////////////////////////////
	$pdf->SetWidths($anchos);  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
	$pdf->SetAligns($alineaciones);  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNA$ClsFalla = new ClsFalla();
	$result = $ClsFalla->get_falla('', $activo, $sede, $sector, $area, $situacion, $desde, $hasta);$i=1;
	if(is_array($result)){
			...
			if(is_array($columnas)){
				foreach($columnas as $col){
					...
					if($col == "act_codigo"){
						...
						$arrcampos[$j] = cambia_fechaHora($row["fall_fecha_falla"]);
					}else if($col == "fall_fecha_registro"){
						$arrcampos[$j] = cambia_fechaHora($row["fall_fecha_registro"]);
					}else if($col == "fall_fecha_solucion"){
						$sit = trim($row["fall_situacion"]);
						$fecha = cambia_fechaHora($row["fall_fecha_solucion"]);
						$arrcampos[$j] = ($sit == 2)?$fecha:'-';
					}else if($col == "fall_situacion"){
						$sit = trim($row["fall_situacion"]);
						$arrcampos[$j] = ($sit == 1)?'Reportado':'Solucionado';
					}else{
	...
	$pdf->Output("Reporte de Fallas","I");?>