...
$id = $_SESSION["codigo"];
	...
	$titulo = "Reporte de Auditoria";////////////////////////////////////// PARAMETROS ///////////////////////////////////////////
   $anchos[1] = 6;
   $alineaciones[1] = "C";
   $titulos[1] = "No.";
   $i = 2;
...
$objPHPExcel->setActiveSheetIndex(0);    /// Combinacion de Celdas (Merge cells)
   ...
   for($i = 1; $i <= $totalColumnas; $i++){
	...
	$result = $ClsEje->get_ejecucion('',$auditoria,'',$sede,$departamento,$categoria,$desde,$hasta,$situacion);
	if(is_array($result)){
		$i = 1;
			...
			if(is_array($columnas)){
				foreach($columnas as $col){
					...
					if($col == "eje_codigo"){
						...
						$campo = ($campo == 1)?'En Proceso':'Finalizada';
						$objPHPExcel->getActiveSheet()->setCellValue($letra.$linea, $campo);
