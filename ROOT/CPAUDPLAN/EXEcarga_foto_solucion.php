...
// obtenemos los datos del archivo
	...
	$ClsPla = new ClsPlan();// Upload
	if($archivo != "") {
		$fotCodigo = $ClsPla->max_foto($auditoria,$pregunta,$ejecucion);
		$fotCodigo++;
		$stringFoto = str_shuffle($auditoria.$pregunta.$ejecucion.uniqid());
		$sql = $ClsPla->insert_foto($fotCodigo,$auditoria,$pregunta,$ejecucion,$stringFoto);
		$rs = $ClsPla->exec_sql($sql);
		if($rs == 1){
			// guardamos el archivo a la carpeta files
			$destino =  "../../CONFIG/Fotos/SOLUCION/".$stringFoto.".jpg";
	...
	}$result = $ClsPla->get_fotos('',$ejecucion,$auditoria,$pregunta);
	$arrFoto = array();
	$foto = "";
	$i = 0;
	if(is_array($result)){
		foreach ($result as $row){
			...
			if(file_exists('../../CONFIG/Fotos/SOLUCION/'.$foto.'.jpg') || $foto != ""){
				$arrFoto[$i]["foto"] = '<img onclick="menuFoto('.$fotCodigo.','.$auditoria.','.$pregunta.','.$ejecucion.');" class="img-upload" src="../../CONFIG/Fotos/SOLUCION/'.$foto.'.jpg" alt="..." />';
			}else{
