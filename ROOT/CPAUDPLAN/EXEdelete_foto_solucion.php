...
// obtenemos los datos del archivo
	...
	if($fotCodigo != "" && $auditoria != "" && $ejecucion != "" && $pregunta) {
		$result = $ClsPla->get_fotos($fotCodigo,$ejecucion,$auditoria,$pregunta);
		$ultimaFoto = "";
		if(is_array($result)){
		...
		//--
		$sql = $ClsPla->delete_foto($fotCodigo,$auditoria,$pregunta,$ejecucion);
		$rs = $ClsPla->exec_sql($sql);
		if($rs == 1){
		///eliminamos la anterior
		unlink("../../CONFIG/Fotos/SOLUCION/".$ultimaFoto.".jpg");
