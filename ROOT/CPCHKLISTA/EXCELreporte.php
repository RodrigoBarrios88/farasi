...
$id = $_SESSION["codigo"];
	...
	$titulo = "Reporte de Listas";////////////////////////////////////// PARAMETROS ///////////////////////////////////////////
   $anchos[1] = 6;
   $alineaciones[1] = "C";
   $titulos[1] = "No.";
   $i = 2;
...
$objPHPExcel->setActiveSheetIndex(0);    /// Combinacion de Celdas (Merge cells)
   ...
   for($i = 1; $i <= $totalColumnas; $i++){
	...
	$result = $ClsLis->get_programacion($codigo,$lista,$sede,$sector,$area,$categoria,$dia,'',$sit);
	if(is_array($result)){
		$i = 1;
			...
			if(is_array($columnas)){
				foreach($columnas as $col){
					...
					if($col == "list_codigo"){
						...
						$dias .= ($diaMes != 0) ? "dia $diaMes del mes " : ""; 
