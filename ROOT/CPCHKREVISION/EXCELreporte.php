...
$id = $_SESSION["codigo"];
	...
	$titulo = "Reporte de Revisiones";////////////////////////////////////// PARAMETROS ///////////////////////////////////////////
   $anchos[1] = 6;
   $alineaciones[1] = "C";
   $titulos[1] = "No.";
   $i = 2;
			...
			if(is_array($columnas)){
				foreach($columnas as $col){
					...
					if($col == "rev_codigo"){
						...
						$campo = 'Anexo A No. # '.Agrega_Ceros($row["rev_codigo"]).' (Click aqui)';
						$objPHPExcel->getActiveSheet()->setCellValue($letra.$linea, trim($campo));
						$url = url_origin( $_SERVER )."/ROOT/CPCHKREVISION/FRMimagenes.php?codigo=$codigo&firma=1&foto=0";
						$objPHPExcel->getActiveSheet()->getCell($letra.$linea)->getHyperlink()->setUrl($url);
					}else if($col == "rev_foto"){
						$codigo = trim($row["rev_codigo"]);
						$campo = 'Anexo B No. # '.Agrega_Ceros($row["rev_codigo"]).' (Click aqui)';
						$objPHPExcel->getActiveSheet()->setCellValue($letra.$linea, trim($campo));
						$url = url_origin( $_SERVER )."/ROOT/CPCHKREVISION/FRMimagenes.php?codigo=$codigo&foto=1&firma=0";
						$objPHPExcel->getActiveSheet()->getCell($letra.$linea)->getHyperlink()->setUrl($url);
					}else{
						$campo = trim($row[$campo]);
