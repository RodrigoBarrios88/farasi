...
$id = $_SESSION["codigo"];
	...
	$pdf->MultiCell(0, 5, 'REPORTE DE REVISIONES DE INDICADORES', 0 , 'L' , 0);
   ...
   $pdf->SetTextColor(0);  // AQUI LE DOY COLOR AL TEXTO
	...
	$result = $ClsRev->get_revision_indicador('', '', '', $usuario, $clasificacion, $departamento, $categoria, $desde, $hasta, '1,2');
		...
		foreach($result as $row){
			...
			if(is_array($columnas)){
				foreach($columnas as $col){
					...
					if ($col == "rev_codigo" || $col == "ind_codigo" || $col == "cat_codigo" || $col == "cla_codigo" || $col == "cla_unidad_medida" || $col == "dep_codigo") {
