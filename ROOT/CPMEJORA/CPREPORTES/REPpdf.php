...
$pdf->SetXY(10, 35);
$pdf->Cell(195, 6, utf8_decode('Plan de Acción'), 10, 0, 'C', true);
...
/////---------- Actividades ----------------
$pdf->SetXY(10, $Y);
$pdf->SetWidths(array(25, 110, 30, 30));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
$pdf->SetAligns(array('C', 'l', 'C', 'C'));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNA// EN EL ARRAY, CADA DATO ES UNA COLUMNA, IGUAL SE HACE PARA INGRESAR LOS DATOS
...
$pdf->Row(array('Codigo', 'Actividad', 'Fecha de Inicio', 'Fecha Final'));

////////////////////////////////////// CUERPO ///////////////////////////////////////////
$pdf->SetWidths(array(25, 110, 30, 30));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
$pdf->SetAligns(array('C', 'L', 'C', 'C'));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNA$ClsUsu = new ClsUsuario();
$ClsAct = new ClsActividad();
//var_dump($plan);
$result = $ClsAct->get_programacion_mejora("", $plan);
//var_dump($result);
$i = 1;
if (is_array($result)) {
	foreach ($result as $row) {
		...
		//descripcion
		$nombre = utf8_decode($row["act_descripcion"]);
		//fecha de inicio
		...
		$no = $i . ".";
		$pdf->SetX(10);
		$pdf->Row(array($codigo, $nombre, $fini, $ffin)); // AGREGO LOS DATOS A LA FILA, VIENE REPERESENTADO POR UN ARRAY 
