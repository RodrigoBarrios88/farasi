...
if ($archivo != "") {
	...
	$id = $ClsAct->max_archivo_mejora();
	$id++;
	$sql = $ClsAct->insert_archivo_mejora($id, $codigo, $posicion, $stringFoto);
	$rs = $ClsAct->exec_sql($sql);
	if ($rs == 1) {
		// vemos si el destino es la imagen o el documento
		$destino = ($posicion == 1) ? "../../CONFIG/Fotos/MEJORA/" . $stringFoto . ".jpg" : "../../CONFIG/Archivos/MEJORA/" . $stringFoto . ".pdf";
