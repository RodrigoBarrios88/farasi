...
$id = $_SESSION["codigo"];
	...
	$pdf->SetAligns(array('C', 'L'));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNA$ClsNor = new ClsNorma();
	$result = $ClsNor->get_norma($codigo,'',1);$i=1;
	if(is_array($result)){
		foreach($result as $row){
			//nombre
			$nombre = utf8_decode($row["nor_nombre"]);
			//---
