...
include_once('html_fns_reportes.php');
	...
	// $pdf=new PDF('P','mm','Letter');  // si quieren el reporte Vertical$pdf->Open();
	$pdf->AddPage();
	$pdf->SetMargins(5,5,5);
	$pdf->Ln(2);

	$pdf->SetFont('Arial','B',12);
	$pdf->MultiCell(0, 5, 'REPORTE DE PERMISOS', 0 , 'L' , 0);
	$pdf->SetFont('Arial','',12);
	$pdf->MultiCell(0, 6, 'Fecha/Hora de generacion: '.date("d/m/Y H:i"), 0 , 'L' , 0);
	$pdf->MultiCell(0, 5, 'Generado por: '.$nombre_sesion, 0 , 'L' , 0);
	$pdf->MultiCell(0, 5, 'Sucursal: '.$sucursal, 0 , 'L' , 0);
	$pdf->Image('../../img/logo.jpg' , 245 ,5, 45 , 30,'JPG', '');

	$pdf->Ln(10);
	////////////////////////////////////// ENCABEZADOS ///////////////////////////////////////////
	$pdf->SetWidths(array(10, 30, 60, 135, 35));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
	...
	for($i=0;$i<1;$i++){  // ESTE ES EL ENCABEZADO DE LA TABLA, 
		$pdf->Row(array('No.', 'Codigo', 'Grupo', 'Permiso','Clave'));
	}

	////////////////////////////////////// CUERPO ///////////////////////////////////////////
	$pdf->SetWidths(array(10, 30, 60, 135, 35));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
	$pdf->SetAligns(array('C', 'C', 'L', 'L', 'C'));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNA$ClsPerm = new ClsPermiso();
	$result = $ClsPerm->get_permisos($id,$grupo,$desc,$clv);$i=1;
	if(is_array($result)){
		foreach($result as $row){
			...
			$no = $i.".";
			$pdf->Row(array($no,$codigo,$grud,$nom,$clv)); // AGREGO LOS DATOS A LA FILA, VIENE REPERESENTADO POR UN ARRAY 
