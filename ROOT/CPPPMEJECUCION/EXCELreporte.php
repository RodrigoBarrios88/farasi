...
$id = $_SESSION["codigo"];
	...
	$doctitle = "REP_TAREAS";////////////////////////////////////// PARAMETROS ///////////////////////////////////////////
   $anchos[1] = 6;
   $alineaciones[1] = "C";
   $titulos[1] = "No.";
   $i = 2;
...
$objPHPExcel->setActiveSheetIndex(0);    /// Combinacion de Celdas (Merge cells)
   ...
   for($i = 1; $i <= $totalColumnas; $i++){
	...
	$result = $ClsPro->get_programacion($codigo,$activo,$usuario,'',$sede, '', $area, $desde, $hasta,'','',$situacion);
	if(is_array($result)){
		$i = 1;
			...
			if(is_array($columnas)){
				foreach($columnas as $col){
					...
					if($col == "act_codigo"){
						...
						$campo = trim($row["mon_simbolo"]).'. '.trim($row[$col]);
						$objPHPExcel->getActiveSheet()->setCellValue($letra.$linea, $campo);
