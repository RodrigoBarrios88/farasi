...
$ClsFic = new ClsFicha();
$result = $ClsFic->get_ficha("", $tipo, $usuario, $fini, $ffin, "", $situacion);
	...
	foreach ($result as $row) {
		...
		if (is_array($columnas)) {
			foreach ($columnas as $col) {
				...
				} else if ($col == "fic_tipo") {
					$campo = trim($row["tit_nombre"]) . " " . trim($row["sub_nombre"]);
					$objPHPExcel->getActiveSheet()->setCellValue($letra . $linea, $campo);
				} else if ($col == "fic_fecha_registro" || $col == "fic_fecha_update" || $col == "fic_fecha_aprobacion") {
					$campo = cambia_fechaHora($row[$campo]);
					$objPHPExcel->getActiveSheet()->setCellValue($letra . $linea, $campo);
				} else if ($col == "fic_situacion") {
					switch ($row[$campo]) {
						case 1:
							$campo = 'En Edición';
							break;
