...
if ($request != "") {
	switch ($request) {
		case "modificar":
			...
			modifica_control($codigo, $descripcion);
			break;
		default:
			$payload = array(
				"status" => false,
...
function modifica_control($codigo, $descripcion)
{
	...
	if ($codigo != "" ) {
		$sql = $ClsCon->modifica_control($codigo, $descripcion); //El codigo se remplazará por el tipo, ya que solo existirá un control por tipo y por sistema
