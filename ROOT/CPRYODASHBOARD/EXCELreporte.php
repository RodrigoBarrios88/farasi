		...
		if (is_array($columnas)) {
			foreach ($columnas as $col) {
				...
				if ($col == "rie_codigo") {
					...
					$campo = get_probabilidad_excel($probabilidad);
					$objPHPExcel->getActiveSheet()->setCellValue($letra . $linea, trim($campo));
				} else if ($col == "rie_impacto") {
					$impacto = $row["rie_impacto"];
					$campo = utf8_decode(get_impacto($impacto));
					$objPHPExcel->getActiveSheet()->setCellValue($letra . $linea, trim($campo));
					...
					$impacto = $row["rie_impacto"];
					$probabilidad = trim($row["rie_probabilidad"]);
					$campo = get_condicion_excel(intval($impacto) * intval($probabilidad));
					$objPHPExcel->getActiveSheet()->setCellValue($letra . $linea, trim($campo));
					...
					$objPHPExcel->getActiveSheet()->setCellValue($letra . $linea, utf8_decode(trim($campo)));
				} else {
					$campo = trim($row[$campo]);
					$objPHPExcel->getActiveSheet()->setCellValue($letra . $linea, trim($campo));
				}
