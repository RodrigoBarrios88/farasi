...
function get_tabla($codigo){ 
	...
	$result = $ClsEnc->get_categoria($codigo);
	if(is_array($result)){
		$arr_respuesta = array(
			"status" => true,
...
function grabar_categoria($nombre,$color){
	$ClsEnc = new ClsEncuesta();
	if($nombre != "" && $color != ""){
		$codigo = $ClsEnc->max_categoria();
		$codigo++; /// Maximo codigo de Version
		$sql = $ClsEnc->insert_categoria($codigo,$nombre,$color); /// Inserta Version
...
function modificar_categoria($codigo,$nombre,$color){
	...
	if($codigo != "" && $nombre != "" && $color != ""){
		$sql = $ClsEnc->modifica_categoria($codigo,$nombre,$color);
		$rs = $ClsEnc->exec_sql($sql);
		if($rs == 1){
			$arr_respuesta = array(
				"status" => true,
...
function situacion_categoria($codigo,$situacion){ 
	$ClsEnc = new ClsEncuesta();
	$sql = $ClsEnc->cambia_situacion_categoria($codigo,$situacion);
	$rs = $ClsEnc->exec_sql($sql);
	if($rs == 1){
		$arr_respuesta = array(
			"status" => true,
