...
$codigo_cuestionario = $ClsEnc->decrypt($hashkey1, $usuario);
$codigo_invitacion = $ClsEnc->decrypt($hashkey2, $usuario);
$ejecucion = $ClsEnc->decrypt($hashkey3, $usuario);
$result = $ClsEnc->get_secciones($seccion, $codigo_cuestionario, 1);
if (is_array($result)) {
	$i = 1;
...
if (is_array($result)) {
	...
	foreach ($result as $row) {
		...
		$correo = utf8_decode($row["sed_nombre"]);
		$direccion = utf8_decode($row["sed_direccion"]) . ", " . utf8_decode($row["correo_municipio"]);
		$cliente = utf8_decode($row["dep_nombre"]);
								...
								if (is_array($result)) {
									...
									foreach ($result as $row) {
										...
										if ($pregunta_tipo == 1) {
											...
											$salida = ""; ///limpia la cadena por cada vuelta
											$salida .= '<div class="btn-group btn-group-toggle" data-toggle="buttons" >';
											$salida .= '<label class="btn btn-white ' . $respSI . '" id="labelSI' . $pregunta_codigo . '" ' . $disabled . ' onclick="responderPonderacion(\'' . $codigo_cuestionario . '\',\'' . $pregunta_codigo . '\',\'' . $ejecucion . '\',\'' . $seccion_codigo . '\',3,1);">';
											$salida .= '<input type="radio" name="options" id="optSI' . $pregunta_codigo . '" autocomplete="off"> <i class="fa fa-check"></i> SAT';
											$salida .= '</label>';
											//--
											$salida .= '<label class="btn btn-white ' . $respNO . '" id="labelNO' . $pregunta_codigo . '" ' . $disabled . ' onclick="responderPonderacion(\'' . $codigo_cuestionario . '\',\'' . $pregunta_codigo . '\',\'' . $ejecucion . '\',\'' . $seccion_codigo . '\',3,2);">';
											$salida .= '<input type="radio" name="options" id="optNO' . $pregunta_codigo . '" autocomplete="off"> NO SAT <i class="fa fa-times"></i>';
											$salida .= '</label>';
											$salida .= '</div>';
										}

										$result = $ClsRes->get_fotos('', $ejecucion, $codigo_cuestionario, $pregunta_codigo);
										$strFoto = "";
										$foto = "";
										if (is_array($result)) {
											foreach ($result as $row) {
												...
												if (file_exists('../../CONFIG/Fotos/AUDITORIA/' . $foto . '.jpg') || $foto != "") {
													$strFoto .= '<img onclick="menuFoto(' . $fotCodigo . ',' . $codigo_cuestionario . ',' . $pregunta_codigo . ',' . $ejecucion . ');" class="img-upload" src="../../CONFIG/Fotos/AUDITORIA/' . $foto . '.jpg" alt="...">';
												} else {
												...
												<div class="row">
													...
													<div class="col-md-3 col-xs-3 text-right">
														<div data-toggle="buttons-checkbox" class="btn-group">
															<button class="btn btn-warning <?php echo $aplicaActive; ?>" id="aplica<?php echo $pregunta_codigo; ?>" onkeyup="texto(this)" onclick="responderAplica('<?php echo $codigo_cuestionario; ?>','<?php echo $pregunta_codigo; ?>','<?php echo $ejecucion; ?>','<?php echo $seccion_codigo; ?>','<?php echo $pregunta_tipo; ?>');" type="button">
