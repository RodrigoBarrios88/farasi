			...
			foreach ($aprobados as $rowAprobado) {
				...
				if (is_array($result)) {
					foreach ($result as $row) {
						...
						// Ejecutado
						$ejecutado += intval($ClsAct->count_programacion_mejora("", $plan, "", "", "3,4,5"));
						// Pendiente
						$pendiente += intval($ClsAct->count_programacion_mejora("", $plan, "", "", "", date("d/m/Y")));
						$pendiente += intval($ClsAct->count_programacion_mejora("", $plan, "", date("d/m/Y"), "1,2"));
						// Vencido
						$vencido += intval($ClsAct->count_programacion_mejora("", $plan, "", "", "1,2", "", date("d/m/Y")));
					}
				}
			}
		}
			...
			for ($origenTipo = 1; $origenTipo <= 6; $origenTipo++) {
				...
				switch ($origenTipo) {
					case 1:
						...
						break;
					case 2:
						$hallazgos = $ClsHal->get_hallazgo_auditoria_externa("", "", $proceso);
						break;
					case 3:
						$hallazgos = $ClsHal->get_hallazgo_queja("", "", $proceso);
						break;
					case 4:
						$hallazgos = $ClsHal->get_hallazgo_indicador("", "", $proceso);
						break;
					case 5:
						$hallazgos = $ClsHal->get_hallazgo_riesgo("", "", $proceso);
						break;
					case 6:
						// $hallazgos = $ClsHal->get_hallazgo_riesgo("", "", $proceso);
						break;
				...
				if (is_array($hallazgos)) {
					foreach ($hallazgos as $rowHallazgo) {
						...
						$result = $ClsPla->get_plan_mejora("", $hallazgo,"","","",3);
						if (is_array($result)) {
							foreach ($result as $row) {
								$plan = trim($row["pla_codigo"]);
								// Ejecutado
					...
					for ($origenTipo = 1; $origenTipo <= 6; $origenTipo++) {
						...
						switch ($origenTipo) {
							case 1:
								...
								break;
							case 2:
								$hallazgos = $ClsHal->get_hallazgo_auditoria_externa("", "", $proceso, $sistema);
								break;
							case 3:
								$hallazgos = $ClsHal->get_hallazgo_queja("", "", $proceso, $sistema);
								break;
							case 4:
								$hallazgos = $ClsHal->get_hallazgo_indicador("", "", $proceso, $sistema);
								break;
							case 5:
								$hallazgos = $ClsHal->get_hallazgo_riesgo("", "", $proceso, $sistema);
								break;
						}
						if (is_array($hallazgos)) {
							foreach ($hallazgos as $rowHallazgo) {
