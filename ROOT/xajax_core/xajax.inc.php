...
class xajax
{
	...
	*/
	var $bCleanBuffer;/*
		String: sLogFile	A configuration setting tracked by the main <xajax> object.  Set the name of the
	...
	*/
	...
	/*
		Constructor: xajaxConstructs a xajax instance and initializes the plugin system.
		
		Parameters:sRequestURI - (optional):  The <xajax->sRequestURI> to be used
	...
	*/
	function __wakeup()
	{
		ob_start();$sLocalFolder = dirname(__FILE__);
...
//EndSkipAIO// this is the list of folders where xajax will look for plugins
...
//EndSkipAIO
		...
		$this->objPluginManager->loadPlugins($aPluginFolders);$this->objLanguageManager = xajaxLanguageManager::getInstance();
	...
	/*
		Function: getGlobalResponseReturns the <xajaxResponse> object preconfigured with the encoding
		and entity settings from this instance of <xajax>.  This is used
		for singleton-pattern response development.Returns:<xajaxResponse> : A <xajaxResponse> object which can be used to return
	...
	*/
	...
	/*
		Function: getVersionReturns:string : The current xajax version.
	*/
	function getVersion()
	{
		return 'xajax 0.5';
...
Parameters:
	...
	*/
	...
	{
		$aArgs = func_get_args();
		$nArgs = func_num_args();if (2 < $nArgs)
		{
			if (XAJAX_PROCESSING_EVENT == $aArgs[0])
			{
				$sEvent = $aArgs[1];
				$xuf = $aArgs[2];		if (false == is_a($xuf, 'xajaxUserFunction'))
...
Parameters:
	...
	*/
	function configure($sName, $mValue)
	{
		...
		$this->objResponseManager->configure($sName, $mValue);$this->aSettings[$sName] = $mValue;
	}

	/*
		Function: configureMany
		
		Set an array of configuration options.Parameters:
	...
	*/
	...
	/*
		...
		via <xajax->configure> or <xajax->configureMany>Parameters:
	...
	*/
	...
	/*
		Function: processRequestIf this is a xajax request (see <xajax->canProcessRequest>), call the
		requested PHP function, build the response and send it back to the
		browser.This is the main server side engine for xajax.  It handles all the
		...
		your script.This function may exit, if a request is processed.  See <xajax->bAllowExit>
	*/
...
//EndSkipDebugif ($this->canProcessRequest())
		{
			...
			$mResult = true;	// handle beforeProcessing event
			if (isset($this->aProcessingEvents[XAJAX_PROCESSING_EVENT_BEFORE]))
			{
				...
				$mResult = $this->objPluginManager->processRequest();	if (true === $mResult)
			{
			...
			else if (is_string($mResult))
			{
				...
				// and send a debug message.		$this->objResponseManager->clear();
				$this->objResponseManager->append(new xajaxResponse());		// handle invalidRequest event
	...
	*/
	...
	/*
		Function: autoCompressJavascriptCreates a new xajax_core, xajax_debug, etc... file out of the
		...
		Parameters:sJsFullFilename - (string):  The relative path and name of the file
			to be compressed.
	...
	*/
	...
	{
		$sJsFile = 'xajax_js/xajax_core.js';if ($sJsFullFilename) {
	...
	function _compressSelf($sFolder=null)
	{
		...
		require_once(dirname(__FILE__) . '/xajaxCompress.inc.php');if ($handle = opendir($sFolder)) {
	...
	function _compile($sFolder=null, $bWriteFile=true)
	{
		...
		$aOutput = array();if ($handle = opendir($sFolder)) {
			while (!(false === ($sName = readdir($handle)))) {
				if ('.' != $sName && '..' != $sName && is_dir($sFolder . '/' . $sName)) {
	...
	/*
		Function: _detectURIReturns the current requests URL based upon the SERVER vars.Returns:string : The URL of the current request.
	*/
	function _detectURI() {
		$aURL = array();// Try to get the request URL
		...
		$sURL.= $aURL['host'];// Add the port if needed
		if (!empty($aURL['port']) 
			&& (($aURL['scheme'] == 'http' && $aURL['port'] != 80) 
			|| ($aURL['scheme'] == 'https' && $aURL['port'] != 443))) {
			$sURL.= ':'.$aURL['port'];
		}// Add the path and the query string
		$sURL.= $aURL['path'].@$aURL['query'];// Clean up
		unset($aURL);
		...
		if (1 < count($aURL))
		{
			$aQueries = explode("&", $aURL[1]);	foreach ($aQueries as $sKey => $sQuery)
			{
	...
	*/
	...
	/*
		Function: setCharEncodingSets the character encoding that will be used for the HTTP output.
		...
		Parameters:sEncoding - (string):  The encoding to use.
	...
	*/
	...
	/*
		Function: getCharEncodingReturns the current character encoding.  See also <xajax->setCharEncoding>
		and <XAJAX_DEFAULT_CHAR_ENCODING>Returns:string : The character encoding.Note:
		deprecated : This function will be removed in future versions.  Please
	...
	*/
	...
	/*
		Function: setFlagsSets a series of flags.  See also, <xajax->setFlag>.
		
		Parameters:flags - (array):  An associative array containing the name of the flag
	...
	*/
	...
	/*
		Function: setFlagSets a single flag (boolean true or false).Available flags are as follows (flag, default value):
			- debug, false
			- verbose, false
			- statusMessages, false
			- waitCursor, true
		...
		Parameters:name - (string): The name of the flag to set.
	...
	*/
	...
	/*
		Function: getFlagReturns the current value of the flag.  See also <xajax->setFlag>.
		
		Parameters:name - (string):  The name of the flag.Returns:boolean : The value currently associated with the flag.Note:
	...
	*/
	...
	/*
		Function: setRequestURISets the URI to which requests will be sent.
		
		Parameters:sRequestURI - (string):  The URINote: $xajax->setRequestURI("http://www.xajaxproject.org");deprecated : This function will be removed in future versions.  Please
			use <xajax->configure> instead.
	*/
	...
	/*
		Function: getRequestURIReturns:string : The current request URI that will be configured on the client
			side.  This is the default URI for all requests made from the current
			page.  See <xajax->setRequestURI>.Note:
		deprecated : This function will be removed in future versions.  Please
			use <xajax->getConfiguration> instead.
	*/
	...
	/*
		Function: setDefaultModeSets the default mode for requests from the browser.
		
		Parameters:sDefaultMode - (string):  The mode to set as the default.	- 'synchronous'
			- 'asynchronous'Note:
		deprecated : This function will be removed in future versions.  Please
			use <xajax->configure> instead.
	*/
	...
	/*
		Function: getDefaultModeGet the default request mode that will be used by the browser
		for submitting requests to the server.  See also <xajax->setDefaultMode>Returns:string - The default mode to be used by the browser for each
			request.
	...
	*/
	...
	/*
		Function: setDefaultMethodSets the default method for making xajax requests.
		
		Parameters:sMethod - (string):  The name of the method.	- 'GET'
			- 'POST'
		Note:
		deprecated : This function will be removed in future versions.  Please
			use <xajax->configure> instead.
	*/
	...
	/*
		Function: getDefaultMethodGets the default method for making xajax requests.Returns:string - The current method configured.
	...
	*/
	...
	/*
		Function: setWrapperPrefixSets the prefix that will be prepended to the javascript wrapper
		functions.  This allows a little flexibility in setting the naming
		for the wrapper functions.
		
		Parameters:sPrefix - (string):  The prefix to be used.
			- default is 'xajax_'Note:
		deprecated : This function will be removed in future versions.  Please
			use <xajax->configure> instead.
	*/
	...
	/*
		Function: getWrapperPrefixGets the current javascript wrapper prefix.  See also, <xajax->setWrapperPrefix>Returns:string - The current wrapper prefix.
	...
	*/
	...
	/*
		Function: setLogFileSpecifies a log file that will be written to by xajax during a
		...
		Parameters:sFilename - (string):  The full or reletive path to the log file.Note:
		deprecated : This function will be removed in future versions.  Please
			use <xajax->configure> instead.
	*/
	...
	/*
		Function: getLogFileReturns the current log file path.  See also <xajax->setLogFile>.Returns:string : The log file path.Note:
		deprecated : This function will be removed in future versions.  Please
			use <xajax->getConfiguration> instead.
	*/
	function getLogFile()
	...
	/*
		Function: registerFunctionRegisters a PHP function or method with the xajax request processor.  This
		makes the function available to the browser via an asynchronous
		(or synchronous) javascript call.
		
		Parameters:mFunction - (string or array):  The string containing the function name
	...
	*/
	...
	/*
		Function: registerCallableObjectRegisters an object whose methods will be searched for a match to the
		...
		Parameters:oObject - (object, by reference):  The object whose methods will be
			registered.
	...
	*/
	...
	/*
		Function: registerEventAssigns a callback function with the specified xajax event.  Events
		are triggered during the processing of a request.List: Available events:
		...
		Parameters:mCallback - (function): The function or object callback to be assigned.
		sEventName - (string): The name of the event.Note:
		deprecated : This function will be removed in future versions.  Please
			use <xajax->register> instead.
	*/
...
function xajaxErrorHandler($errno, $errstr, $errfile, $errline)
{
	...
	if (($errno & $errorReporting) == 0) return;if ($errno == E_NOTICE) {
		$errTypeStr = 'NOTICE';
	}
