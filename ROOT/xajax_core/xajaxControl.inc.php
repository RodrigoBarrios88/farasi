...
/*
	Constant: XAJAX_HTML_CONTROL_DOCTYPE_FORMATDefines the doctype of the current document; this will effect how the HTML is formatted
	when the html control library is used to construct html documents and fragments.  This can
	be one of the following values:'XHTML' - (default)  Typical effects are that certain elements are closed with '/>'
...
//EndSkipDebug$this->aAttributes[$sName] = $sValue;
	}/*
...
//EndSkipDebug$this->aEvents[$sEvent] = array(
			&$objRequest, 
			$aParameters, 
			$sBeforeRequest, 
			$sAfterRequest
...
//EndSkipDebug$sClass = $this->getClass();
...
//EndSkipDebug
	...
	function _printEvents()
	{
		foreach (array_keys($this->aEvents) as $sKey)
		{
			...
			$sAfterRequest = $aEvent[3];	foreach ($aParameters as $aParameter)
			{
...
/*
	Class: xajaxControlContainerThis class is used as the base class for controls that will contain
...
class xajaxControlContainer extends xajaxControl
{
	...
	*/
	...
	{
		xajaxControl::xajaxControl($sTag, $aConfiguration);$this->clearChildren();
...
//EndSkipDebug$this->aChildren[] = $objControl;
	}
